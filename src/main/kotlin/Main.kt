...
fun genRandom(problem: Task, dist: Int): Solve {
    val size = problem.musicians.size
    var cells = (0..(problem.stage_width.toInt() - 20) step 10).flatMap { x ->
    ...
    val coefX =
        problem.stage_bottom_left[0] + problem.stage_width - cells.maxOfOrNull { it.x }!! - cells.minOfOrNull { it.x }!!
    val c1 = cells.filter {
        val a = min(
    ...
    val c2 = cells.filter {
        ...
        min(a, b) < (dist + 1) * 10.0
    }.filter { it !in c1 }.shuffled().take(size - c1.size)
